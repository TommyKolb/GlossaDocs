...
4. **On-screen keyboard defaults** — Register a built-in layout in `src/app/utils/keyboardLayouts.ts`: define a `KeyboardLayout` constant and add it to `LANGUAGE_KEYBOARD_LAYOUTS`. Add at least one unit test in `src/test/unit/keyboard-layouts.test.ts` for that layout (structure or a known key mapping). Extend `backend/src/modules/input-preferences/keyboard-layout-overrides-schema.ts` so `keyboardLayoutOverrides` in API validation includes the new language key (`en` / `de` / `ru` pattern).
5. **User keyboard overrides** — No extra DB migration: overrides are stored as JSON. New languages only need the built-in layout + schema allowlist above.
